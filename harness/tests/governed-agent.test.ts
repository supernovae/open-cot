...
describe("runGovernedAgent (mock backend)", () => {
  beforeEach(() => {
    resetStepCounter();
  });

  function config(overrides: Partial<GovernedAgentConfig> = {}): GovernedAgentConfig {
  ...
  it("runs through governed FSM with tool calls", async () => {
    const result = await runGovernedAgent(config());
