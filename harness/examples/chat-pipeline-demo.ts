#!/usr/bin/env npx tsx
/**
 * Chat cognitive pipeline demo — run with `npx tsx examples/chat-pipeline-demo.ts`
 *
 * By default uses the mock backend (zero external deps).
 * Set OPENAI_BASE_URL and OPENAI_API_KEY to use a real LLM.
 *
 * Examples:
 *   npx tsx examples/chat-pipeline-demo.ts                     # mock backend
 *   OPENAI_BASE_URL=http://localhost:11434/v1 \
 *     npx tsx examples/chat-pipeline-demo.ts                   # Ollama
 */

import { runChatPipeline } from "../src/pipelines/chat-pipeline.js";
...
async function main() {
  ...
  console.log(`\n--- Chat Cognitive pipeline Demo ---`);
  console.log(`Question: ${question}\n`);

  const backend = pickBackend();
  const trace = await runChatPipeline(backend, question, createMockToolRegistry());
