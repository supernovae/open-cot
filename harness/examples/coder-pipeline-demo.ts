#!/usr/bin/env npx tsx
/**
 * Coder cognitive pipeline demo — run with `npx tsx examples/coder-pipeline-demo.ts`
 *
 ...
 *
 * Examples:
 *   npx tsx examples/coder-pipeline-demo.ts                     # mock backend
 *   OPENAI_BASE_URL=http://localhost:11434/v1 \
 *     npx tsx examples/coder-pipeline-demo.ts                   # Ollama
 */
...
async function main() {
  ...
  console.log(`\n--- Coder Cognitive pipeline Demo ---`);
  console.log(`Task: ${task}\n`);

  const backend = pickBackend();
  const trace = await runCoderPipeline(backend, task, createMockToolRegistry());
